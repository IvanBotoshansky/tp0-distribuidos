...
Para que el servidor permita aceptar conexiones y procesar mensajes en paralelo, se utilizó la librería multiprocessing de Python.

Por cada conexión aceptada, se crea un nuevo proceso que se encarga de manejar la comunicación con el cliente que estableció la conexión. De esta forma, el servidor puede atender múltiples clientes de manera concurrente.

#### Herramientas de sincronización

- Respecto al almacenamiento y carga de las apuestas, se utilizó un Lock para garantizar que solo un proceso pueda acceder al archivo de apuestas a la vez. En caso contrario, se podrían generar inconsistencias en los datos.

- Para la actualización del registro de agencias que terminaron de enviar apuestas, se utilizó un diccionario compartido entre los procesos con `manager.dict()`. Este diccionario en realidad es un proxy devuelto por el `Manager` que permite a los procesos compartir información de manera segura y que cada operacion sobre el diccionario sea atómica. Sin embargo, se tiene un Lock para el conjunto de operaciones de modificar el diccionario y ver si todas las agencias terminaron de enviar sus apuestas (ver la longitud del diccionario). Esto es así para evitar que un proceso verifique la longitud del diccionario mientras otro lo modifica.

- Para manejar la lógica del sorteo, se utilizó un Event que permite a los procesos esperar hasta que el evento sea seteado. Si un proceso recibe un mensaje `WinnersRequest` y el sorteo no se realizó, espera hasta que el evento sea seteado. El evento será seteado por el proceso que detecte que todas las agencias terminaron de enviar las apuestas. Una vez que el evento es seteado, cada proceso procede a enviar la lista de ganadores a la agencia/cliente que corresponda. Cabe destacar que cada cliente, luego de enviar la petición de ganadores, espera hasta recibir la lista de ganadores por parte del servidor (envían una única vez la petición, a diferencia del ejercicio anterior).

- Para el caso del manejo de la señal SIGTERM, se utilizó una variable compartida entre los procesos que indica si se recibió la señal. Cada proceso verifica si la señal fue recibida y, en caso afirmativo, realiza el cierre graceful cerrando la conexión con el cliente. El proceso principal (padre) es el encargado de setear la variable compartida cuando recibe la señal SIGTERM y esperar a que todos los procesos hijos terminen.